...
这样便可以防止核心API库被随意篡改。可能你会想，如果我们在classpath路径下自定义一个名为java.lang.SingleInterge类(该类是胡编的)呢？该类并不存在java.lang中，经过双亲委托模式，传递到启动类加载器中，由于父类加载器路径下并没有该类，所以不会加载，将反向委托给子类加载器加载，最终会通过系统类加载器加载该类。但是这样做是不允许，因为java.lang是核心API包，需要访问权限，强制加载将会报出如下异常
...
2.Java虚拟机栈：线程私有的，用于存储局部变量表，操作栈，方法返回值。局部变量表放着基本数据类型，还有对象的引用。
...
[基本类型变量、引用类型变量的在java中的存放位置](https://www.cnblogs.com/dongtian-blogs/p/10668460.html)
 一、局部变量存储在方法栈中
...
String s1 = "china"; s1是对象的引用，存放在栈中   "china"存放在常量池中。（这种编译期已经确定好)
String ss1 = new String("china"); 首先去检查常量池中有没有china。如果没有则在常量池中创建一个此字符串对象，然后堆中再创建一个常量池中此”china”对象的拷贝对象。

这也就是有道面试题：String s = new String(“xyz”);产生几个对象？一个或两个，如果常量池中原来没有”xyz”,就是两个。

final类型的存储在常量池。

static 存储在方法区，jdk1.7永久代，jdk1.8元空间
...
初始标记--》并发标记--》重新标记--》并发清除
（初始标记、重新标记）仍需STW。但初始标记仅仅只标记了一下GC Roots能直接关联到的对象，速度很快。
...
浮动垃圾：由于CMS并发清理阶段用户线程还在运行着，伴随着程序运行自然就会有新的垃圾不断产生，这部分垃圾出现的标记过程之后，
CMS无法在当次收集中处理掉它们，只好留待下一次GC中再清理。这些垃圾就是“浮动垃圾”。

3）CMS是一款“标记--清除”算法实现的收集器，容易出现大量空间碎片。当空间碎片过多，将会给大对象分配带来很大的麻烦，
...
## CMS收集器和G1收集器的区别
1.范围：CMS收集器是老年代的收集器，可以配合新生代的Serial和ParNew收集器一起使用
G1收集器收集范围是老年代和新生代。不需要结合其他收集器使用
2. CMS垃圾碎片，G1没有。  CMS标记清除   G1标记整理
3. CMS最停顿时间，G1最大吞吐量。
...
为了防止内存溢出：开启大型文件或从数据库一次拿了太多的数据很容易造成内存溢出，所以在这些地方要大概计算一下数据量的最大值是多少，并且设定所需最小及最大的内存空间值。 
