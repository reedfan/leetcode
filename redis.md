# Table of Contents
  ...
  * [使用过Redis做异步队列？](#使用过redis做异步队列？)
    ...
    * [如何保证缓存与数据库的双写一致性？](#如何保证缓存与数据库的双写一致性？)
    * [如何保证缓存与数据库的双写一致性？](#如何保证缓存与数据库的双写一致性？-1)
...
读取数据的时候，如果发现数据不在缓存中，根据唯一标识路由之后，也发送同一个 jvm 内部队列中。一个队列对应一个工作线程，每个工作线程串行拿到对应的操作，然后一条一条的执行。
